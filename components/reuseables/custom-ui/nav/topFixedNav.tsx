...
const TopNavBar = () => {
  ...
  return (
     <header
      className={`fixed left-0 right-0 top-0 z-50 flex h-14 w-full items-center justify-center  font-bold ${isHomePage ? (isScrolled ? 'bg-black text-white' : 'bg-black bg-opacity-10') : 'bg-black text-white'} cursor-pointer transition-colors duration-300`}
    >
      <nav className='flex-no-wrap container z-40 flex h-24 w-full flex-row items-center justify-between gap-4 sm:px-12'>
        ...
        >
          {isMenuOpen ? <FaTimes /> : <RiMenu2Fill />}
        </button>
        {isMenuOpen && (
          <nav
        ...
        <Link href='/' className='flex'>
          <BiWorld className='h-12 w-12' />
          <span className='ml-1 mt-2 text-2xl'>
